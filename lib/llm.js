...
async function executeTool(name, input) {
  switch (name) {
    ...
    case 'list_artifacts': {
      ...
      return { artifacts: arts.map(a => ({ id: a.id, type: a.type, created: a.created, metadata: a.metadata })) };
    }
...
LONG RESPONSES: If your response would be more than ~5 lines or contains structured data (tables, lists, reports, breakdowns), use generate_markdown to store it as an artifact instead. Your chat response should be a 1-line summary. The markdown viewer loads automatically in the Transaction Layer.

DEEP LINKS: When referencing a viewable artifact, include the link format: localhost:3000/?viewer=VIEWER&artifact=ID so the user can bookmark/share it.
