...
function loadProjectConfig() {
  const configPath = process.env.PROJECT_CONFIG_PATH || path.join(__dirname, '..', 'project.json');
  if (fs.existsSync(configPath)) {
    return JSON.parse(fs.readFileSync(configPath, 'utf8'));
  }
  return { name: 'Forkless', slug: 'forkless', protocols: './protocols/internal/' };
}

function buildSystemPrompt() {
  const config = loadProjectConfig();
  const protocolDir = config.protocols
    ? path.resolve(path.dirname(process.env.PROJECT_CONFIG_PATH || path.join(__dirname, '..', 'project.json')), config.protocols)
    : path.join(__dirname, '..', 'protocols', 'internal');
  ...
  const brandName = config.name || 'Forkless';
  return `You are the ${brandName} agent on the Forkless platform.
